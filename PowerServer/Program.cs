...
using PowerAPI.PowerDB;
using Console = PowerAPI.PowerConsole.Console;
...
namespace PowerServer
{
    ...
    {
        public static HttpServer webServer; // A HTTP server
        public static Console console; // The PowerConsole class to make life easier
        [STAThread]
        public static void Main(string[] args) // Program entry point
        {
            try
            {
                console = new("PowerServer"); // Initializing the ConsoleTools
                if (args.Length > 1 || args.Length == 1 && args[0] == "/help") // Check if it has help/incorrect params
                {
                    console.Help("PowerServer", new string[] { "Argument 1: Path to settings file (optional)", @"Example: PowerServer.exe C:\SettingsFile.json" }); // Show help menu
                    console.WaitForKeyPress(); // Wait for key press
                    return; // Exit
                }
                console.Header("PowerServer", "Version 1.0"); // Show app header
                ...
                if (ApplicationExtensions.IsRunningAsAdmin() && !Directory.Exists(Path.Combine(ApplicationExtensions.GetApplicationLocation(), "Users"))) { Directory.CreateDirectory(Path.Combine(ApplicationExtensions.GetApplicationLocation(), "Users")); } // Create Users directory if dosent exist
                if (ApplicationExtensions.IsRunningAsAdmin() && !Directory.Exists(Path.Combine(ApplicationExtensions.GetApplicationLocation(), "Website"))) { Directory.CreateDirectory(Path.Combine(ApplicationExtensions.GetApplicationLocation(), "Website")); } // Create Website directory if dosent exist
                webServer = new(80, "http://localhost", Path.Combine(ApplicationExtensions.GetApplicationLocation(), "Website")); // Initializing the HTTP server
                ...
                if (settings.EnableWebServer && ApplicationExtensions.IsRunningAsAdmin()) { webServer.Start(); } // Start HTTP server case enabled on server settings

                while (true) // App loop
                {
                    switch (console.Select(new string[] { (settings.EnableDB || !ApplicationExtensions.IsRunningAsAdmin() ? "Disable" : "Enable") + " PowerDB", (settings.EnableWebServer || !ApplicationExtensions.IsRunningAsAdmin() ? "Disable" : "Enable") + " PowerWeb", "Run PowerSend", "Save config file", "Exit" })) // Show selection prompt
                    {
                        ...
                        case 3: // Case to run email sending wizard
                            string email = console.Prompt("What is the email address that is going to be used?"); // Asks user "What is the email address that is going to be used?"
                            string displayName = console.Prompt("What is the display name that is going to be used?"); // Asks user "What is the display name that is going to be used?"
                            string password = console.Prompt("What is the application key for that email address?"); // Asks user "What is the application key for that email address
                            string header = console.Prompt("What is the email header going to be?"); // Asks user "What is the email header going to be?"
                            string content = console.Prompt(@"What is the email content going to be (use \n for newlines)?").Replace(@"\n", "\n"); // Asks user "What is the email content going to be (use \n for newlines)?"
                        ...
                        case 4: // Case to export settings to file
                            ...
                            string fileName = console.Prompt("What filename do you want to export it to?"); // Asks user "What filename do you want to export it to?"
                            fileName = fileName == "" ? "settings" : fileName; // In case of null file name, set it to "settings"
                            string enableDBPrompt = console.Prompt("Would you like to run PowerDB?"); // Asks user "Would you like to run PowerDB?"
                            while (!new string[] { "Yes", "Y", "yes", "y", "No", "N", "no", "n" }.Contains(enableDBPrompt)) // Checks if anwser is "Yes", "Y", "yes", "y", "No", "N", "no" or "n"
                            {
                                console.WriteLine("Invalid anwser. Valid anwsers are: Yes, Y, yes, y, No, N, no, n", ConsoleColor.Red); // Writes "Invalid anwser. Valid anwsers are: Yes, Y, yes, y, No, N, no, n" with the color red
                                enableDBPrompt = console.Prompt("Would you like to run PowerDB?"); // Asks user "Would you like to run PowerDB?"
                            }
                            ...
                            string enableWebServerPrompt = console.Prompt("Would you like to run PowerWeb?"); // Asks user "Would you like to run PowerWeb?"
                            while (!new string[] { "Yes", "Y", "yes", "y", "No", "N", "no", "n" }.Contains(enableWebServerPrompt)) // Checks if anwser is "Yes", "Y", "yes", "y", "No", "N", "no" or "n"
                            {
                                console.WriteLine("Invalid anwser. Valid anwsers are: Yes, Y, yes, y, No, N, no, n", ConsoleColor.Red); // Writes "Invalid anwser. Valid anwsers are: Yes, Y, yes, y, No, N, no, n" with the color red
                                enableWebServerPrompt = console.Prompt("Would you like to run PowerWeb?"); // Asks user "Would you like to run PowerWeb?"
                            }
                    ...
                    console.Clear(4); // Clear last selection
                }
            }
            catch { }
        }

        public static string DBPostRequestHandler(string request, string requestUrl) // Handle Post Request using DB
        {
